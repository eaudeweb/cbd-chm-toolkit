...
function open_framework_form_system_theme_settings_alter(&$form, &$form_state) {
  ...
  $form['site_country'] = array(
    ...
    '#description' => t('Specifies the country code to enable auto path generation'),
    '#collapsible' => TRUE,
    '#collapsed' => FALSE,
  );
  
  $form['site_country']['site_country_id'] = array(
    '#type'          => 'textfield',
    '#title'         => t('Site Country ID'),
    '#default_value' => theme_get_setting('site_country_id'),
    '#description'   => t("2 digit Country ID"),
  );
  
  $form['site_theme'] = array(
    '#type' => 'fieldset',
    '#title' => t('Site Theme'),
    '#description' => t('Use these settings to adjust the site theme.'),
    '#collapsible' => TRUE,
    '#collapsed' => FALSE,
  );
  
  $form['site_theme']['global_site_classes'] = array(
    '#type'          => 'radios',
    '#title'         => t('Choose between various themes'),
    '#default_value' => theme_get_setting('global_site_classes'),
    '#options'       => array(
      'green-theme ' => t('White Header and Green Navigation - <strong><em>Default</em></strong>'),
	  'light-gray-theme ' => t('Light Gray Header and White Navigation'),
    ),
  );
  // Responsive Behavior
